...
export const getters = {
  ...
  getCMS: state => {
    ...
    return state.cmsData.filter(function(row) {
      return (
        regex.test(row.name) || regex.test(row.body) || regex.test(row.author)
      );
    });
  },
  getDocuments: state => {
  ...
  getIsCountry: state =>
    state.landingPageData && state.landingPageData.isCountry,
...
export const actions = {
  ...
  async search({ rootGetters, commit, dispatch }, code) {
    try {
      ...
      const params =
        code && code.length > 2
          ? {
              donor: rootGetters["system/getDonors"].find(
  ...
  async cmsSearch({ state, commit }) {
    try {
      ...
      const { data } = await this.$axios.get("/api/cms/", {
        params: { search: state.searchString }
      });
  ...
  async loadCountryData({ commit, dispatch, rootGetters }, code) {
    try {
      ...
      commit(
        ...
        Object.freeze({ isCountry: true, ...data })
      );
    } catch (e) {
  ...
  async loadDonorData({ commit, rootGetters }, code) {
    try {
      ...
      commit(
        "SET_LANDING_PAGE_DATA",
        Object.freeze({ isCountry: false, ...data })
      );
    } catch (e) {
      console.error(e);
