...
if ! [ -x "$(command -v docker-compose -f docker-compose.yml -f docker-compose.prod.yml)" ]; then
  echo 'Error: docker-compose -f docker-compose.yml -f docker-compose.prod.yml is not installed.' >&2
  exit 1
fi
