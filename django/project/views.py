    ...
    def list_all(request, *args, **kwargs):
        ...
        result_list = functools.reduce(
            lambda acc, p: acc + [{
                ...
                "health_focus_areas": [str(x) for x in HealthFocusArea.objects.get_names_for_ids(p.data.get("health_focus_areas", []))],
                "hsc_challenges": [str(x) for x in HSCChallenge.objects.get_names_for_ids(p.data.get("hsc_challenges", []))],
                ...
                "platforms": [str(x) for x in TechnologyPlatform.objects.get_names_for_ids([x['id'] for x in p.data.get("platforms", [])])],
            }], projects, [])
