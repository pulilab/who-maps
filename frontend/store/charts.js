...
export const actions = {
  ...
  async getDashboardData(
    ...
    // const totalProjects = {
    //   published: projectStatus.map(x => (x.published)).reduce((partial_sum, a) => partial_sum + a,0),
    //   draft: projectStatus.map(x => (x.draft)).reduce((partial_sum, a) => partial_sum + a,0),
    //   growth: projectStatus.map(x => (x.growth)).reduce((partial_sum, a) => partial_sum + a,0),
    //   ready_to_publish: projectStatus.map(x => (x.ready_to_publish)).reduce((partial_sum, a) => partial_sum + a,0),
    //   to_delete: projectStatus.map(x => (x.to_delete)).reduce((partial_sum, a) => partial_sum + a,0),
    //   unpublished: projectStatus.map(x => (x.unpublished)).reduce((partial_sum, a) => partial_sum + a,0),
    // }
    const totalProjects = projectStatus
      .map(x => x.published)
      .reduce((partial_sum, a) => partial_sum + a, 0);
    // const totalProjects = extract(projectStages, 'total').reduce((acc, val) => acc + val, 0)+noStageDataSum;
