...
const run = ($rootScope, $state, $mdToast, $mdDialog, $ngRedux, $timeout, $transitions, gettextCatalog) => {
    ...
    const showPopUp = () => {
        $mdToast.show(
          $mdToast.simple()
            .textContent(gettextCatalog('Ops! something went wrong, try again later.'))
    ...
    const showCredentialsPopUp = async () => {
        ...
        const message = gettextCatalog('You are not logged in or your session has expired, please login again.');
        await $mdDialog.show(
          $mdDialog.alert()
            .clickOutsideToClose(false)
            .title(gettextCatalog('Authentication problem'))
            .textContent(message)
            .theme('alert')
            .ariaLabel('Auth problem dialog')
            .ok(gettextCatalog('Understand'))
        );
