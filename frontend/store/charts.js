...
export const actions = {
    getStageData({ commit, rootState }, stages) {
        ...
        const { color, rotation, dash, point } = phaseInfo(
            lastLabelType(labels)
        )
                ...
                callbacks: {
                    ...
                    footer: (tooltipItem, data) => {
                        ...
                        if (xLabel.includes('Ended')) {
                            return `Note: ${notes[notes.length - 1]}`.match(
                                /.{1,38}/g
                            )
                        }
                        return notes[yLabel]
    ...
    async getDashboardData(
        ...
        const kpi = await Promise.all([
            this.$axios.get(
                `${base}/users/${objectToQueryString(state.filters)}`
            ),
            this.$axios.get(
                `${base}/tokens/${objectToQueryString(state.filters)}`
            ),
            this.$axios.get(
            ...
            this.$axios.get(
                `${base}/hfa/${objectToQueryString(state.filters)}`
            ),
            this.$axios.get(
                `${base}/health-categories/${objectToQueryString(
                    state.filters
                )}`
            ),
        ])
        ...
        hfaCoveredSubAreas.forEach(hfaSub => {
            if (lastHfaCategoryID !== hfaSub.hfaCategory.id) {
                ...
                hfaCoveredAreas[hfaCoveredAreas.length - 1].subareas.push(
                    hfaSub.name
                )
            }
        })
        ...
        const dataStandardsLabels = extract(
            totalsOfStandardsSorted,
            'name',
            true
        )
        ...
        commit(
            ...
            settings({
                ...
                labels: extract(projectStatus, 'date').map(d =>
                    format(d, 'YYYY-MMM')
                ),
        ...
        commit(
            ...
            legendGenerator(
                ['Monthly User Growth', 'Monthly Active User'],
                colorSetB
            )
        )
