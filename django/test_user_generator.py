...
def generate_users():
    for x in range(1,number_of_users):
        ...
        headers = {
            "Authorization": "Token {}".format(response.json().get("key"))
        }
        ...
        project_data.update(name=project_pattern.format(x), organisation=organisation_pattern.format(x), country=(x%10)+1)
