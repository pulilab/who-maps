...
class KPITokensTests(KPITestData, APITestCase):
    ...
    def test_token_kpi_nofilter(self):
        ...
        expected = \
            [{'country': self.country_global.id, 'date': self.date_1_str, 'tokens': 3},
             {'country': self.country_global.id, 'date': self.date_2_str, 'tokens': 2}]
    ...
    def test_token_kpi_time_filter(self):
        ...
        response = self.client.get(url)
        expected = \
            [{'date': self.date_2_str, 'country': self.country_global.id, 'tokens': 2}]
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected)
