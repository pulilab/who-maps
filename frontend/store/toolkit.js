...
export const getters = {
  ...
  getAxisDetail: (state, getters, rootState, rootGetters) => axisId => {
    const axisStructure = rootGetters['system/getAxis'].find(a => a.id === axisId);
    const systemDomains = rootGetters['system/getDomains'];
    const axisData = getters.getToolkitData.find(a => a.id === axisId);
    const domains = axisData ? axisData.domains.map((add, index) => ({
      ...add,
...
export const actions = {
  async loadToolkitData ({commit, rootState}) {
    try {
      ...
      if (projectId) {
        const { data } = await this.$axios.get(`/api/projects/${projectId}/toolkit/data/`);
  ...
  async saveAnswer ({commit, dispatch, rootState}, answer) {
    try {
      const projectId = rootState.projects.currentProject;
      if (projectId) {
        const { data } = await this.$axios.post(`/api/projects/${projectId}/toolkit/score/`, answer);
...
export const mutations = {
  ...
  UPDATE_TOOLKIT_DATA: (state, data) => {
    state.toolkitData[data.axis].domains[data.domain].questions[data.question].answers[data.answer] = data.value;
  },
